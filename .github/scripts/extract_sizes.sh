...
if [ -f "build/partition_table/partition-table.bin" ]; then
  PARTITION_TABLE_SIZE=$(stat -c%s "build/partition_table/partition-table.bin")
...
if [ -n "$MAIN_BINARY" ] && [ -f "$MAIN_BINARY" ]; then
  APP_SIZE=$(stat -c%s "$MAIN_BINARY")
  echo "Main binary ($MAIN_BINARY) size: $APP_SIZE bytes"
fi

# Calculate total size
TOTAL_SIZE=$((BOOTLOADER_SIZE + PARTITION_TABLE_SIZE + APP_SIZE))
